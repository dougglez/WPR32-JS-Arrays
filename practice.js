...
function divider(numbersArray) {
  ...
  let odds = numbersArray.filter(num => num % 2); 
  return [evens, odds];
}
...
function addTen(numbers) {
  return numbers.map(v => Number(v)+10);
}
...
//Code Here
devMountainEmployees = devMountainEmployees.filter(v => v.name != 'Cahlan');
...
//Code Here
users.push(  user1, {name: 'D', email: 'D', password: 'D', username: 'D'}, {name: 'D', email: 'D', password: 'D', username: 'D'})
...
//Code Here
users = users.filter(v => v.email != "tylermcginnis33@gmail.com");
